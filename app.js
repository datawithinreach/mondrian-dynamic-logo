...
function playTransition(canvas, targetLayout) {
  ...
  function renderFrame(now) {
    ...
    // D. If lockup, render aligned wordmark text (font-size 76, baseline adjusted to y=154 to center all-caps Antonio perfectly)
    if (isLockup) {
      const text = document.createElementNS('http://www.w3.org/2000/svg', 'text');
      text.setAttribute('x', '140');
      text.setAttribute('y', '154'); // Adjusted baseline to account for Antonio cap-height centering
